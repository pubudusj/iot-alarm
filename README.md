...
```bash
iot-alarm$ sam build --use-container
```
...
```bash
iot-alarm$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
iot-alarm$ sam logs -n HelloWorldFunction --stack-name iot-alarm --tail
```
...
```bash
iot-alarm$ pip install pytest pytest-mock --user
iot-alarm$ python -m pytest tests/ -v
```
